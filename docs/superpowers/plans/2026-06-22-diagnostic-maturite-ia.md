# Diagnostic de maturité IA — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ajouter `/outils/diagnostic-maturite-ia` — quiz 12 questions × 4 options, 6 dimensions, radar SVG en résultats, PostHog complet.

**Architecture:** Outil standalone `kind: 'app'`, données hardcodées dans `KitDiagnosticMaturite.vue`, branché via le système existant `outils/[slug].vue`. Pas d'API, pas de données en frontmatter.

**Tech Stack:** Nuxt 3, Vue 3 `<script setup>`, CSS vars DA V2 Editorial Dark, SVG inline, `usePosthogEvent()` composable.

---

## Task 1 — Scaffolding statique (MD + manifest + prerender + wiring)

**Files:**
- Create: `content/outils/diagnostic-maturite-ia.md`
- Modify: `app/data/outils-manifest.ts`
- Modify: `nuxt.config.ts`
- Modify: `app/pages/outils/[slug].vue`

- [ ] **Step 1 : Créer le fichier MD**

Créer `content/outils/diagnostic-maturite-ia.md` avec exactement ce contenu :
...
- [ ] **Step 2 : Ajouter l'entrée dans le manifest**

Dans `app/data/outils-manifest.ts`, ajouter dans le tableau `OUTILS_MANIFEST` **après** l'entrée `le-grand-filtre` :
...
- [ ] **Step 3 : Ajouter la route prerender**

Dans `nuxt.config.ts`, dans le tableau `routes` de `nitro.prerender`, ajouter après la ligne `'/outils/le-grand-filtre',` :

```ts
        '/outils/diagnostic-maturite-ia',
```

- [ ] **Step 4 : Brancher le composant dans [slug].vue**

Dans `app/pages/outils/[slug].vue`, dans la section `<div class="kit-body prose">`, ajouter **après** le bloc `KitGenerateurBpmn` (et avant `<MDC v-if="kit.outro"`) :
...
- [ ] **Step 5 : Vérifier que le scaffolding compile**
...
Attendu : pas d'erreur sur les nouveaux fichiers (le composant `KitDiagnosticMaturite` sera introuvable mais Nuxt ne compile les composants qu'à l'usage — l'erreur apparaîtra au runtime seulement si on visite la page).

- [ ] **Step 6 : Commit**

```bash
git add content/outils/diagnostic-maturite-ia.md app/data/outils-manifest.ts nuxt.config.ts app/pages/outils/[slug].vue
git commit -m "feat(diagnostic-maturite): scaffolding statique — MD, manifest, prerender, wiring"
```

---

## Task 2 — KitDiagnosticMaturite.vue : quiz stage

**Files:**
- Create: `app/components/kits/KitDiagnosticMaturite.vue`

Cette tâche crée le composant complet avec quiz fonctionnel. Les stages `decrypting` et `result` afficheront un placeholder pour l'instant.

- [ ] **Step 1 : Créer le composant**

Créer `app/components/kits/KitDiagnosticMaturite.vue` :
...
- [ ] **Step 2 : Démarrer le dev server et vérifier le quiz**

```bash
cd /Users/mathieu/Documents/survivor && npm run dev
```

Naviguer vers `http://localhost:3000/outils/diagnostic-maturite-ia`.

Vérifier :
- La page se charge sans erreur console
- Les 12 questions s'affichent dans l'ordre (avec l'eyebrow `OBJECTIF · CAS D'USAGE` pour Q1)
- La barre de progression avance à chaque réponse
- Le bouton "← Q PRÉCÉDENTE" apparaît à partir de Q2
- L'animation decrypting s'affiche immédiatement après la Q12
- Après 1,2s, le placeholder result s'affiche

- [ ] **Step 3 : Commit**

```bash
git add app/components/kits/KitDiagnosticMaturite.vue
git commit -m "feat(diagnostic-maturite): quiz stage — 12 questions, 4 options, progress, PostHog partiel"
```

---

## Task 3 — Result stage : palier + ladder + score + priorités + PostHog complet

**Files:**
- Modify: `app/components/kits/KitDiagnosticMaturite.vue`

- [ ] **Step 1 : Ajouter les constantes LEVELS et ADVICE dans le composant**

Dans la section `<script setup>`, après `OPTION_KEYS`, ajouter :
...
- [ ] **Step 2 : Ajouter le computed `result`**

Après la constante `total = QUESTIONS.length`, ajouter :
...
- [ ] **Step 3 : Compléter la fonction `answer()` avec PostHog `completed` et `result_viewed`**

Remplacer le bloc `else` de la fonction `answer()` (après `idx.value + 1 < total`) par :
...
- [ ] **Step 4 : Compléter `restart()` avec PostHog**

Remplacer la fonction `restart()` par :
...
- [ ] **Step 5 : Remplacer le bloc result-placeholder dans le template**

Remplacer le bloc `<!-- RESULT (placeholder) -->` par :
...
```vue
        ...
        <div v-else-if="stage === 'result'" class="result">
          ...
          <div class="result-panel radar-panel">
            <p style="color: var(--color-muted); font-family: var(--font-mono); font-size: 0.75rem; text-align: center;">
              // RADAR — Task 4
            </p>
          </div>
...
- [ ] **Step 6 : Ajouter les styles result**

Dans `<style scoped>`, ajouter après les styles existants :
...
- [ ] **Step 7 : Vérifier les résultats dans le navigateur**

Le dev server doit être en cours. Refaire le quiz complet et vérifier :
- Le palier s'affiche avec la bonne couleur
- La ladder montre le bon palier surligné
- Le score `/100` s'affiche
- Les priorités (dimensions faibles) s'affichent avec le texte conseil
- Le bouton "refaire" remet le quiz à zéro
- Le placeholder radar indique `// RADAR — Task 4`

- [ ] **Step 8 : Commit**

```bash
git add app/components/kits/KitDiagnosticMaturite.vue
git commit -m "feat(diagnostic-maturite): result stage — palier, ladder, score, priorités, PostHog complet"
```
...
- [ ] **Step 1 : Ajouter les helpers radar dans `<script setup>`**

Après `const ADVICE`, ajouter :
...
- [ ] **Step 2 : Remplacer le placeholder radar dans le template**

Remplacer le bloc `<!-- Radar (placeholder) -->` par :
...
- [ ] **Step 3 : Ajouter les styles radar**

Dans `<style scoped>`, **supprimer** la règle `.radar-panel { min-height: 120px; display: grid; place-items: center; }` (la classe `.radar-panel` reste dans le template mais n'a plus besoin de sizing explicite — le contenu radar remplit naturellement). Puis **ajouter** :
...
- [ ] **Step 4 : Vérifier le radar dans le navigateur**

Refaire le quiz. Vérifier que :
- Le radar SVG s'affiche avec les 6 axes
- Le polygone se trace selon les scores
- Les points sur chaque axe sont colorés (rouge/amber/vert) selon le score de la dimension
- La légende à droite est lisible
- Sur mobile (< 640px), le radar et la légende s'empilent correctement

- [ ] **Step 5 : Commit**

```bash
git add app/components/kits/KitDiagnosticMaturite.vue
git commit -m "feat(diagnostic-maturite): radar SVG 6 axes + légende par dimension"
```

---

## Task 5 — Vérification finale et build

**Files:** aucun fichier modifié

- [ ] **Step 1 : Vérifier la page dans la liste des outils**

Naviguer vers `http://localhost:3000/outils`.

Vérifier que la carte "Diagnostic de maturité IA" apparaît dans la grille avec le bon titre et sous-titre.

- [ ] **Step 2 : Tester le flux complet**

Répondre aux 12 questions avec des scores volontairement bas (toujours option A) → vérifier Palier 0 + 3 priorités affichées.

Refaire avec scores hauts (toujours option D) → vérifier Palier 4 + section priorités absente (aucune dimension < 1.5).

- [ ] **Step 3 : Vérifier les events PostHog dans les Dev Tools**

Ouvrir Network > XHR/Fetch, filtrer sur `posthog` ou `batch`. Faire le quiz et vérifier que les events suivants apparaissent dans les requêtes :
- `kit_diagnostic_started`
- `kit_diagnostic_question_answered` (×12)
- `kit_diagnostic_completed`
- `kit_diagnostic_result_viewed`

- [ ] **Step 4 : Build de prod**
...
Attendu : build sans erreur TypeScript ni warnings bloquants.

- [ ] **Step 5 : Vérifier le prerender**

```bash
ls /Users/mathieu/Documents/survivor/.output/public/outils/diagnostic-maturite-ia/
```

Attendu : un fichier `index.html` présent.

- [ ] **Step 6 : Commit final si tout est vert**

```bash
git add app/components/kits/KitDiagnosticMaturite.vue
git commit -m "feat(diagnostic-maturite): outil complet — quiz + radar + PostHog + build vérifié"
```
