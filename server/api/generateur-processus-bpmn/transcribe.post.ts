...
const NAMESPACE = 'generateur-processus-bpmn-transcribe'

export default defineEventHandler(async (event) => {
  const config = useRuntimeConfig()

  const ip = getRequestIP(event, { xForwardedFor: true }) ?? 'unknown'
  const rl = checkRateLimit(ip, { namespace: NAMESPACE })
  if (!rl.allowed) {
  ...
  if (!token || !productId) {
    console.error('[generateur-processus-bpmn/transcribe] Infomaniak config missing')
  ...
  try {
    ...
    console.error('[generateur-processus-bpmn/transcribe] Infomaniak upload failed:', err instanceof Error ? err.message : err)
