...
1. **Les données contiennent-elles des données personnelles ?** (noms de locataires ou d'investisseurs, coordonnées, identifiants…)
...
2. **Peut-on anonymiser durablement les données avant traitement** (réidentification à effort disproportionné) ?
   - Oui : les données sortent du champ de la nLPD — large flexibilité. Rare en pratique pour des données immobilières ou financières, souvent réidentifiables par recoupement.
...
3. **Données sensibles, secret particulier, ou traitement dans le périmètre de l'externalisation FINMA ?**
   - Oui : exigences renforcées — résidence suisse + DPA obligatoires, fournisseur souverain à privilégier. Évaluer la notification FINMA (Circ. 2018/3) et le plan de sortie.
...
|---|---|
| ChatGPT perso (Free / Plus) | Le cauchemar du Shadow IT. L'entreprise n'a aucun contrôle d'accès ni visibilité sur ce que les employés copient-collent. |
...
- **Secret d'affaires :** juridiquement protégé par les clauses de confidentialité du DPA, la LCD (art. 6) et le Code pénal (art. 162) ; mais ces protections ne tiennent pas face à une injonction étrangère qui les court-circuite.
...
- **Périmètre :** un agent de codage touche du code, pas des données de production. Le garder sur du code et des données de test ou synthétiques — jamais sur des données personnelles ou réglementées réelles (investisseurs, locataires). Surveiller les secrets (clés API, mots de passe) qui ne doivent finir ni dans un prompt ni dans un cache local.
...
- Politique interne d'usage de l'IA + classification des données (quel outil pour quel type de donnée).
